...
def register_search_tools(registry: ToolRegistry) -> None:
    ...
    registry.register_fn("search_code", "Search text/regex patterns in workspace files (equivalent to rg/grep). Uses ripgrep when available with automatic Python fallback — always works even if rg is not installed. Prefer this over run_command with rg or grep.", {"type": "object", "properties": {"query": {"type": "string"}, "path": {"type": "string"}, "include": {"type": "string"}, "case_sensitive": {"type": "boolean"}}, "required": ["query"]}, search_code, read_only=True, tags=["search"])
